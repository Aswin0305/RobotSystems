...
class PicarxImproved(object):
    ...
    def __init__(self, 
        ...
        logging.debug("PicarxImproved initialized and atexit registered.")
    ...
    def set_motor_speed(self, motor, speed):
        ...
        if direction < 0:
            ...
            self.motor_speed_pins[motor].pulse_width_percent(speed)
        logging.debug(f"Motor {motor} ({'Left' if motor==1 else 'Right'}) set to speed {speed} with direction {direction} self.cali_dir_value[motor] {self.cali_dir_value[motor]}")
    ...
    def set_cam_tilt_angle_without_constrain(self,value):
        self.cam_tilt.angle(-1*(value + -1*self.cam_tilt_cali_val))
    ...
    def get_ackerman_inner_wheel_speed(self, speed, angle):
        radians = math.radians(abs(angle))
        return speed * math.cos(radians)

    def backward_ackerman(self, speed, angle=None):
        ...
        if current_angle == 0:
            ...
            self.set_motor_speed(1, -self.get_ackerman_inner_wheel_speed(speed, current_angle))
            self.set_motor_speed(2, speed)
        else:
            self.set_motor_speed(1, -speed)
            self.set_motor_speed(2, self.get_ackerman_inner_wheel_speed(speed, current_angle))

        logging.debug(f"backward_ackerman() current_angle {current_angle}")
    ...
    def backward(self, speed):
        ...
        if current_angle != 0:
            ...
            if (current_angle / abs_current_angle) > 0:
                ...
                self.set_motor_speed(2, speed)
            logging.debug(f"current_angle {current_angle} power_scale {power_scale}")
        else:
            self.set_motor_speed(1, -1*speed)
            self.set_motor_speed(2, speed)
        logging.debug(f"backward() current_angle {current_angle}")

    def forward_ackerman(self, speed, angle=None):
        if angle is not None:
            self.set_dir_servo_angle(angle)
        ...
        logging.debug(f"forward_ackerman() current_angle {current_angle}")

    def forward_ackerman_revised(self, speed, angle=None, state='forward'):
        if angle is not None:
            self.set_dir_servo_angle(angle)
        ...
        if current_angle == 0:
            # Go straight - both wheels same speed, opposite directions
            self.set_motor_speed(1, speed)
            self.set_motor_speed(2, -speed)  
        ...
        logging.debug(f"forward_ackerman_revised() current_angle {current_angle}")
    
    def pivot_turn(self, speed, direction):
        """
        Pivot turn in place to recover from sharp line deviation.
        direction: -1 = left pivot, +1 = right pivot
        """
        speed = constrain(speed, 0, 100)

        if direction < 0:
            # Pivot LEFT
            self.set_motor_speed(1, -speed)   # left wheel backward
            self.set_motor_speed(2, -speed)   # right wheel forward (inverted wiring)
        else:
            # Pivot RIGHT
            self.set_motor_speed(1, speed)    # left wheel forward
            self.set_motor_speed(2, speed)    # right wheel backward (inverted wiring)

        logging.debug(f"pivot_turn() direction={'LEFT' if direction < 0 else 'RIGHT'} speed={speed}")
    ...
    def forward(self, speed):
        ...
        if current_angle != 0:
            ...
            logging.debug(f"forward() current_angle {current_angle} power_scale {power_scale}")
        else:
            self.set_motor_speed(1, speed)
            self.set_motor_speed(2, -1*speed)  
            logging.debug(f"forward() current_angle {current_angle}")
    ...
    def parallel_park(self, side='left'):
        ...
        logging.debug(f"Starting parallel park to the {side}")
        # 1. Angle backward
        self.backward_ackerman(40, angle=30 * direction)
        time.sleep(1.5)
        # 2. Counter-steer backward
        self.backward_ackerman(40, angle=-30 * direction)
        time.sleep(1.5)
        # 3. Straighten forward
        self.forward_ackerman(40, angle=0)
        time.sleep(0.5)
        self.stop()

    def k_turn(self, initial_side='left'):
        direction = 1 if initial_side == 'left' else -1
        # 1. Forward Turn
        self.forward_ackerman(40, angle=-30 * direction)
        time.sleep(2.5)
        # 2. Straight
        self.forward_ackerman(40, angle=0)
        time.sleep(1.0)
...
if __name__ == "__main__":
    ...
    try:
        while True:
            print("Commands: [w] forward, [s] backward, [a] forward left, [d] forward right, [z] backward left,[c] backward right, [p] park, [k] k-turn, [8] 8 [q] quit")
            cmd = input("Action >> ").lower()
            if cmd == 'q':
                break
            elif cmd == 'w':
                px.forward_ackerman(50, angle=0)
                time.sleep(1)
                px.stop()
            elif cmd == 's':
                px.backward_ackerman(50, angle=0)
                time.sleep(1)
                px.stop()
            elif cmd == 'a':
                px.forward_ackerman(50, angle=-45)
                time.sleep(1)
                px.stop()
            elif cmd == 'd':
                px.forward_ackerman(50, angle=45)
                ...
                px.backward_ackerman(50, angle=-45)
                time.sleep(1)
                px.stop()
            elif cmd == 'c':
                px.backward_ackerman(50, angle=45)
                ...
                px.forward_ackerman(30, angle=30)
                time.sleep(15)
                px.forward_ackerman(30, angle=-30)
                time.sleep(19)
                px.forward_ackerman(30, angle=30)
